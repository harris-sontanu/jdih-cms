<div class="card-body bg-light pb-0">
    <form action="{{ route('admin.news.index') }}" id="filter-form" class="filter-form" method="GET">
        <div class="row">
            <div class="col-md-3">
                <div class="mb-3">
            ...
            <div class="col-md-3">
                <div class="mb-3">
                    <input type="text" class="form-control" name="body" placeholder="Narasi" value="{{ Request::get('body') }}">
                </div>
            </div>
            <div class="col-md-3">
                <div class="mb-3">
                    <select name="taxonomy" id="taxonomy" class="select">
                        <option value="">Pilih Kategori</option>
                        @foreach ($taxonomies as $key => $value)
                            <option value="{{ $key }}" @selected(Request::get('taxonomy') == $key)>{{ $value }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
