        ...
        <div class="card">
            <div class="card-header card-header d-sm-flex py-sm-0">
                <div class="py-sm-3 mb-sm-0 mb-3">
                    <div class="form-control-feedback form-control-feedback-end">
                        <form action="{{ route('admin.legislation.judgment.index') }}" method="get">
                ...
                @include('admin.legislation.judgment.feature')

            </div>

            <div id="filter-options" @empty (Request::get('filter')) style="display: none" @endempty>

                @include('admin.legislation.judgment.filter')
                        ...
                        <ul class="nav navbar-nav flex-row flex-fill">
                            @foreach ($tabFilters as $key => $value)
                                ...
                                <li class="nav-item me-1">
                                    <a href="{{ route('admin.legislation.judgment.index', ['tab' => $key] + Request::all()) }}" class="navbar-nav-link rounded{{ $active }}">
                            ...
                            <ul class="navbar-nav ms-lg-auto mt-2 mt-lg-0">
                                <li class="nav-item dropdown ms-lg-1">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-end">
                                        <a href="{{ route('admin.legislation.judgment.index', ['limit' => 25] + Request::all()) }}" class="dropdown-item" data-rows="25">25</a>
                                        <a href="{{ route('admin.legislation.judgment.index', ['limit' => 50] + Request::all()) }}" class="dropdown-item" data-rows="50">50</a>
                                        <a href="{{ route('admin.legislation.judgment.index', ['limit' => 100] + Request::all()) }}" class="dropdown-item" data-rows="100">100</a>
                                        <a href="{{ route('admin.legislation.judgment.index', ['limit' => 200] + Request::all()) }}" class="dropdown-item" data-rows="200">200</a>
                                    </div>
                                </li>
                            </ul>
                        </div>
            ...
            <div class="table-responsive">
                <table class="table">
                    <thead>
                        <tr>
                            ...
                            <th class="sorting @if (!empty($sort) AND Request::get('order') == 'category') {{ 'sorting_' . $sort }} @endif">
                                <a href="{{ route('admin.legislation.judgment.index', ['order' => 'category', 'sort' => $sortState] + Request::all()) }}" class="text-dark d-block">Jenis</a>
                            </th>
                            <th class="text-center sorting @if (!empty($sort) AND Request::get('order') == 'number') {{ 'sorting_' . $sort }} @endif">
                                <a href="{{ route('admin.legislation.judgment.index', ['order' => 'number', 'sort' => $sortState] + Request::all()) }}" class="text-dark d-block">Nomor</a>
                            </th>
                            <th class="text-center sorting @if (!empty($sort) AND Request::get('order') == 'year') {{ 'sorting_' . $sort }} @endif">
                                <a href="{{ route('admin.legislation.judgment.index', ['order' => 'year', 'sort' => $sortState] + Request::all()) }}" class="text-dark d-block">Tahun</a>
                            </th>
                            <th class="sorting @if (!empty($sort) AND Request::get('order') == 'title') {{ 'sorting_' . $sort }} @endif">
                                <a href="{{ route('admin.legislation.judgment.index', ['order' => 'title', 'sort' => $sortState] + Request::all()) }}" class="text-dark d-block">Judul</a>
                            </th>
                            <th class="text-center sorting @if (!empty($sort) AND Request::get('order') == 'status') {{ 'sorting_' . $sort }} @endif">
                                <a href="{{ route('admin.legislation.judgment.index', ['order' => 'status', 'sort' => $sortState] + Request::all()) }}" class="text-dark d-block">Status</a>
                            </th>
                            <th class="text-center sorting @if (!empty($sort) AND Request::get('order') == 'user') {{ 'sorting_' . $sort }} @endif">
                                <a href="{{ route('admin.legislation.judgment.index', ['order' => 'user', 'sort' => $sortState] + Request::all()) }}" class="text-dark d-block">Operator</a>
                            </th>
                            <th class="sorting @if (!empty($sort) AND Request::get('order') == 'published_at') {{ 'sorting_' . $sort }} @endif">
                                <a href="{{ route('admin.legislation.judgment.index', ['order' => 'published_at', 'sort' => $sortState] + Request::all()) }}" class="text-dark d-block">Publikasi</a>
                            </th>
                        ...
                        @forelse ($judgments as $judgment)
                            <tr>
                                ...
                                <td>
                                    ...
                                    <abbr class="fs-sm" data-bs-popup="tooltip" title="{{ $judgment->dateFormatted($judgment->published_at, true) }}">{{ $judgment->dateFormatted($judgment->published_at) }}</abbr>
                                </td>
                                <td class="safezone">
                    ...
                    {{ $judgments->links('admin.layouts.pagination') }}
