...
class Legislation extends Model
{
    ...
    public function masterExist(): Attribute
    {
        return Attribute::make(
            get: fn ($value) => $this->documents()->where('type', 'master')->exists()
        );
    }

    public function masterSource(): Attribute
    {
        $master = $this->documents()->where('type', 'master')->latest()->first();

        return Attribute::make(
            get: fn ($value) => empty($master) ? null : Storage::url($master->path)
        );
    }

    public function masterName(): Attribute
    {
        $master = $this->documents()->where('type', 'master')->latest()->first();

        return Attribute::make(
            get: fn ($value) => empty($master) ? null : $master->name
        );
    }

    public function abstractExist(): Attribute
    {
        return Attribute::make(
            get: fn ($value) => $this->documents()->where('type', 'abstract')->exists()
        );
    }

    public function coverId(): Attribute
    {
        $cover = $this->documents()->where('type', 'cover')->latest()->first();

        return Attribute::make(
            get: fn ($value) => empty($cover) ? null : $cover->id
        );
    }

    public function coverName(): Attribute
    {
        $cover = $this->documents()->where('type', 'cover')->latest()->first();

        return Attribute::make(
            get: fn ($value) => empty($cover) ? null : $cover->name
        );
    }
    ...
    {
        $cover = $this->documents()->where('type', 'cover')->latest()->first();
        $coverUrl = asset('assets/admin/images/placeholders/placeholder.jpg');
        if (!empty($cover)) {
            if (Storage::disk('public')->exists($cover->path)) $coverUrl = Storage::url($cover->path);
        }
    ...
    public function coverThumbUrl(): Attribute
    {
        $cover = $this->documents()->where('type', 'cover')->latest()->first();
        $coverThumbUrl = asset('assets/admin/images/placeholders/cover.jpg');
        if (!empty($cover)) {
