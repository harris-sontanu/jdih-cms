            ...
            <div class="card-header card-header d-sm-flex py-sm-0">
                <div class="py-sm-3 mb-sm-0 mb-3">
                    <div class="form-control-feedback form-control-feedback-end">
                        <form action="{{ route('admin.legislation.article.index') }}" method="get">
                ...
                @include('admin.legislation.article.feature')

            </div>

            <div id="filter-options" @empty (Request::get('filter')) style="display: none" @endempty>

                @include('admin.legislation.article.filter')
                        ...
                        <ul class="nav navbar-nav flex-row flex-fill">
                            @foreach ($tabFilters as $key => $value)
                                ...
                                <li class="nav-item me-1">
                                    <a href="{{ route('admin.legislation.article.index', ['tab' => $key] + Request::all()) }}" class="navbar-nav-link rounded{{ $active }}">
                            ...
                            <ul class="navbar-nav ms-lg-auto mt-2 mt-lg-0">
                                <li class="nav-item dropdown ms-lg-1">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-end">
                                        <a href="{{ route('admin.legislation.article.index', ['limit' => 25] + Request::all()) }}" class="dropdown-item" data-rows="25">25</a>
                                        <a href="{{ route('admin.legislation.article.index', ['limit' => 50] + Request::all()) }}" class="dropdown-item" data-rows="50">50</a>
                                        <a href="{{ route('admin.legislation.article.index', ['limit' => 100] + Request::all()) }}" class="dropdown-item" data-rows="100">100</a>
                                        <a href="{{ route('admin.legislation.article.index', ['limit' => 200] + Request::all()) }}" class="dropdown-item" data-rows="200">200</a>
                                    </div>
                                </li>
                            </ul>
                        </div>
                    ...
                    <thead>
                        <tr>
                            ...
                            <th class="sorting @if (!empty($sort) AND Request::get('order') == 'category') {{ 'sorting_' . $sort }} @endif">
                                <a href="{{ route('admin.legislation.article.index', ['order' => 'category', 'sort' => $sortState] + Request::all()) }}" class="text-dark d-block">Jenis</a>
                            </th>
                            <th class="sorting @if (!empty($sort) AND Request::get('order') == 'title') {{ 'sorting_' . $sort }} @endif">
                                <a href="{{ route('admin.legislation.article.index', ['order' => 'title', 'sort' => $sortState] + Request::all()) }}" class="text-dark d-block">Judul</a>
                            </th>
                            <th class="text-center sorting @if (!empty($sort) AND Request::get('order') == 'year') {{ 'sorting_' . $sort }} @endif">
                                <a href="{{ route('admin.legislation.article.index', ['order' => 'year', 'sort' => $sortState] + Request::all()) }}" class="text-dark d-block">Tahun</a>
                            </th>
                            <th class="sorting @if (!empty($sort) AND Request::get('order') == 'source') {{ 'sorting_' . $sort }} @endif">
                                <a href="{{ route('admin.legislation.article.index', ['order' => 'source', 'sort' => $sortState] + Request::all()) }}" class="text-dark d-block">Sumber</a>
                            </th>
                            <th class="sorting @if (!empty($sort) AND Request::get('order') == 'author') {{ 'sorting_' . $sort }} @endif">
                                <a href="{{ route('admin.legislation.article.index', ['order' => 'author', 'sort' => $sortState] + Request::all()) }}" class="text-dark d-block" data-bs-popup="tooltip" title="T.E.U. Orang/Badan">T.E.U.</a>
                            </th>
                            <th class="text-center sorting @if (!empty($sort) AND Request::get('order') == 'user') {{ 'sorting_' . $sort }} @endif">
                                <a href="{{ route('admin.legislation.article.index', ['order' => 'user', 'sort' => $sortState] + Request::all()) }}" class="text-dark d-block">Operator</a>
                            </th>
                            <th class="sorting @if (!empty($sort) AND Request::get('order') == 'published_at') {{ 'sorting_' . $sort }} @endif">
                                <a href="{{ route('admin.legislation.article.index', ['order' => 'published_at', 'sort' => $sortState] + Request::all()) }}" class="text-dark d-block">Publikasi</a>
                            </th>
                    ...
                    <tbody>
                        @forelse ($articles as $article)
                            <tr>
                                ...
                                <td>
                                    <span class="d-block">{{ $article->source }}</span>
                                    <span class="text-muted">{{ $article->place }}</span>
                                </td>
                                ...
                                <td>
                                    ...
                                    <abbr class="fs-sm" data-bs-popup="tooltip" title="{{ $article->dateFormatted($article->published_at, true) }}">{{ $article->dateFormatted($article->published_at) }}</abbr>
                                </td>
                                <td class="safezone">
                                    <div class="d-inline-flex">
                                        <a href="{{ route('admin.legislation.article.show', $article->id) }}" class="text-body mx-1" data-bs-popup="tooltip" title="Lihat"><i class="ph-eye"></i></a>
                                        @if ($onlyTrashed)
                                            @can('restore', $article)
                                                <form action="{{ route('admin.legislation.article.restore', $article->id) }}" method="POST">
                                                    @method('PUT')
                                                    @csrf
                                            ...
                                            @can('forceDelete', $article)
                                                <form class="delete-form" action="{{ route('admin.legislation.article.force-destroy', $article->id) }}" data-confirm="Apakah Anda yakin menghapus artikel {{ $article->title }}?" method="POST">
                                                    @method('DELETE')
                                                    @csrf
