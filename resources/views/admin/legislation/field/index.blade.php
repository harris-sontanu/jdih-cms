                    ...
                    <div class="card-header card-header d-sm-flex py-sm-0">
                        <div class="py-sm-3 mb-sm-0 mb-3">
                            <div class="form-control-feedback form-control-feedback-end">
                                <form action="{{ route('admin.legislation.field.index') }}" method="get">
                        ...
                        <table class="table">
                            <thead>
                                <tr>
                                    ...
                                    <th class="sorting @if (!empty($sort) AND Request::get('order') == 'name') {{ 'sorting_' . $sort }} @endif">
                                        <a href="{{ route('admin.legislation.field.index', ['order' => 'name', 'sort' => $sortState] + Request::all()) }}" class="text-dark d-block">Nama</a>
                                    </th>
                                    <th class="sorting @if (!empty($sort) AND Request::get('order') == 'desc') {{ 'sorting_' . $sort }} @endif">
                                        <a href="{{ route('admin.legislation.field.index', ['order' => 'desc', 'sort' => $sortState] + Request::all()) }}" class="text-dark d-block">Deskripsi</a>
                                    </th>
                                    <th class="text-center sorting @if (!empty($sort) AND Request::get('order') == 'total') {{ 'sorting_' . $sort }} @endif">
                                        <a href="{{ route('admin.legislation.field.index', ['order' => 'total', 'sort' => $sortState] + Request::all()) }}" class="text-dark d-block">Total</a>
                                    </th>
                                    @cannot('isAuthor')                                        
                            ...
                            <tbody id="sortable">
                                @forelse ($fields as $field)
                                    <tr>
                                        <td><span class="fw-semibold d-block">{{ $field->name }}</span></td>
                                        <td>{{ $field->desc }}</td>
                                        <td class="text-center"><span class="badge rounded-pill bg-indigo">{{ $field->legislations->count() }}</span></td>
                                        @cannot('isAuthor')                                            
                                            <td class="safezone">
                                                <div class="d-inline-flex">
                                                    <button type="button" class="btn btn-link text-body p-0" data-bs-popup="tooltip" title="Ubah" data-bs-toggle="modal" data-bs-target="#edit-modal" data-id="{{ $field->id }}" data-name="{{ $field->name }}"><i class="ph-pen"></i></button>
                                                    <form class="delete-form" action="{{ route('admin.legislation.field.destroy', $field->id) }}" data-name="{{ $field->name; }}" method="POST">
                                                        @method('DELETE')
                                                        @csrf
                            ...
                            {{ $fields->links('admin.layouts.pagination') }}
                        </table>
                    </div>
                </div>
            </div>
                    ...
                    <div class="sidebar-content">
                        <div class="card">
                            <div class="sidebar-section-header border-bottom">
                                <span class="fw-semibold">Tambah Bidang Hukum</span>
                            </div>

                            <div class="sidebar-section-body">

                                <form action="{{ route('admin.legislation.field.store') }}" method="post" novalidate>
                                    @csrf
                                    <div class="mb-3">
                                        <label for="name" class="form-label">Nama:</label>
                                        <input id="name" type="text" class="form-control @if ($errors->get('name') OR $errors->get('slug')) is-invalid @endif" name="name" value="{{ old('name') }}" placeholder="Hukum Pidana">
...
@section('script')
    @include('admin.legislation.field.script')
@endsection 