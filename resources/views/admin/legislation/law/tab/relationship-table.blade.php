<div class="table-responsive">
    <table id="{{ $relationType }}-relation-table" class="table">
        ...
        <tbody id="{{ $relationType }}-relation-table-body">
            @if (isset($relationships) AND $relationships->byType($relationType)->isNotEmpty())
                @foreach ($relationships->byType($relationType) as $relation)
                    <tr>
                        ...
                        @if ($relationType !== 'document')
                            <td>
                                ...
                                @empty($law)
                                    <input type="hidden" name="statusRelationship[]" value="{{ $relation->status->value }}">
                                @endempty
                            </td>
                        @endif
                        <td>
                            ...
                            @empty($law)
                                <input type="hidden" name="typeRelationship[]" value="{{ $relationType }}">
                                <input type="hidden" name="relatedTo[]" value="{{ $relation->related_to }}">
                            @endempty
                        </td>
                        <td>
                            {{ $relation->note }}
                            @empty($law)
                                <input type="hidden" name="noteRelationship[]" value="{{ $relation->note }}">
                            @endempty
                        </td>
